...
* standalone (non-Flow) branch here -- on Domino it's read via
* %sysget(DOMINO_IS_WORKFLOW_JOB) and the %include "/mnt/code/domino.sas"
* sets up the adam/sdtm libnames; this bundle's autoexec.sas does the same
* libname setup against mock ADSL/AE/EX data instead. The merge/sort logic
* is untouched.
\*****************************************************************************/
