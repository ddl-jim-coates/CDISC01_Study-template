...
* ----------------------------------------------------------------------------
*
* Jenner bundle note: domino_is_workflow_job is hardcoded to the
* standalone (non-Flow) branch; %include "/mnt/code/domino.sas" is
* replaced by this bundle's autoexec.sas (which stands up mock ADAE/ADSL/
* metadata.t_ae_rel). share/macros/tfl_metadata.sas is inlined verbatim
* below since domino.sas is what normally puts it on SASAUTOS. The ODS
* PDF destination is redirected from /mnt/artifacts/TFL to the run's own
* working directory. The counting/ranking/transpose table logic is
* otherwise untouched.
\*****************************************************************************/

/* share/macros/tfl_metadata.sas, inlined verbatim (normally reached via
   the SASAUTOS path domino.sas sets up). */
%macro tfl_metadata();
	data metadata;
