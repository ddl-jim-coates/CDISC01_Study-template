...
* ----------------------------------------------------------------------------
*
* Jenner bundle note: project_name is hardcoded here -- on Domino it's
* read via %sysget(DOMINO_PROJECT_NAME). The %ensure_directory shell-out
* (x "mkdir -p ...") is dropped since the output path is now the run's
* own working directory, which already exists. The simulated-subject
* loop bound is reduced from 300 to 90 (30 per arm, same 1/3-1/3-1/3
* split) because the unlicensed Jenner engine hard-caps DATA step output
* at 100 rows regardless of OPTIONS OBS -- at the original bound of 300
* the run silently truncates to only the first 100 simulated subjects,
* which are all "Placebo" (i<=100), collapsing the 3-arm table to one
* arm. Nothing else about the simulated-data / table-building logic is
* touched.
\*****************************************************************************/
...
%let output_path = ./&table_name..pdf;
...
data adsl_simulated;
    ...
    /* Create 90 subjects across 3 treatment arms (30 each; reduced from
       300 for this bundle -- see Jenner bundle note above) */
    do i = 1 to 90;
        /* Generate subject ID */
